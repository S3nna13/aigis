...
- **Guardrail consistency**: Normalize `score < threshold` pass semantics across all 14 rails. Previously `ConstitutionalCritique` and `HallucinationDetector` used inverted `score >= threshold` logic (pass when score was high).
- **Hardcoded thresholds**: `ContextWindowGuard` and `StructuredOutputValidator` now accept a `threshold` parameter instead of hardcoding `0.3`.
