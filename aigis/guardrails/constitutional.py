...
PRINCIPLES = [
    ("Helpful and Harmless", ["harmful", "dangerous", "illegal", "unethical"]),
    ("Truthful", ["factually incorrect", "hallucinated", "misleading"]),
    ("Respectful", ["offensive", "hate", "discriminatory", "harassment"]),
    ("Privacy-Safe", ["personally identifiable", "private information"]),
    ("No Deception", ["deceptive", "fraudulent", "manipulative"]),
]
...
class ConstitutionalCritique(Guardrail):
    ...
    def _keyword_check(self, text: str) -> GuardrailResult:
        ...
        score = min(1.0, len(violations) / 3.0)
        passed = score < self.threshold
        reason = (
        ...
        return GuardrailResult(
            ...
            score=score,
            reason=reason,
            severity="critical" if score > 0.7 else "warning" if violations else "info",
        )
